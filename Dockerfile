...
RUN \
	if [ -z ${RELEASE+x} ]; then \
	RELEASE=$(curl -u "${SECRETUSER}:${SECRETPASS}" -sX GET "https://api.github.com/repos/Chia-Network/chia-blockchain/releases/latest" \
	| jq -r ".tag_name"); \
	fi \
	&& git clone --branch ${RELEASE} --recurse-submodules=mozilla-ca https://github.com/Chia-Network/chia-blockchain.git . \
	&& /bin/sh ./install.sh
